defmodule Computer do
  ...
  defp get(0, position, _, data) do
    value = Enum.at(data, position)
    IO.puts "data[#{position}] = #{value}"
    value
  end
  defp get(1, value, _, _) do
    IO.puts "data at current location: #{value}"
    value
  end
  defp get(2, position, rel_base, data) do 
    value = Enum.at(data, rel_base + position)
    IO.puts "data[#{position} + #{rel_base}] = #{value}"
    value
  end
  ...
  def run(idx, rel_base, data, input, output) do
    ...
    IO.puts "Code: #{inspect inst}, Index: #{idx}, Relative Offset: #{inspect rel_base}, Data: #{inspect Enum.slice(data, idx + 1, 3)}"
  ...
  defp process(3, rel_base, modes, idx, [pos | _], data, input, output) do  # Opcode 3 - read input
    ...
    IO.puts "Putting input #{hd(input)} in data[#{val}]"
...
datums = File.read!("day09.txt") |> String.trim() |> String.split(",") |> Enum.map(&String.to_integer/1)